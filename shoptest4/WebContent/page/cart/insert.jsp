...
<c:choose>
 ...
 <c:otherwise>
 <div >
      <form role ="form" method="post" action="wm?command=CartInsert" onsubmit="return check()" class ="form-inline"><!-- action은 servlet의 url pattern과 맞춰줘야함. -->
		   ...
		   상품수량 : <select name="cartQtt" class="form-control">
		                                    <c:forEach begin="1" end="200" var="i">
		                                        <option value="${i}">${i}</option>
		                                    </c:forEach>
		                                </select>&nbsp;개
		     
		  <button type="submit" class="btn btn-sm btn-danger">담기</button>
		</div>
		</form>
	</div>
 </c:otherwise>
